...
of the form $2^p \times 3 f_0$ for integer $p$.

For instance, a perfect fifth, consisting of $f_0$ and $\frac{3}{2} f_0$, interferes
at $3 f_0$.
Likewise, a major third, consisting of $f_0$ and $\frac{6}{5} f_0$, interferes
at $5 f_0$.

\begin{equation}
\boldsymbol{W_{m+1}}[j_1 \ldots j_{m+1}, b] =
\langle \boldsymbol{x_m}[j_1 \ldots j_m, 2^{j_m} b]
\vert
\boldsymbol{\psi_{j_m, b}}
\rangle
\end{equation}

\begin{equation}
\boldsymbol{x_m}[j_1 \ldots j_m, b] =
\big \vert
\boldsymbol{W_{m+1}}[j_1 \ldots j_{m+1}, b]
\big \vert
\end{equation}
...
\begin{equation}
\boldsymbol{S_J x}[j_1 \ldots j_{m}] =
\left(
(\boldsymbol{g_{\downarrow 2}})^{(J - j_m)}
 \circ
\Circ_{n = 1}^{m}
\left \vert
\boldsymbol{h_{\downarrow 2}}
\circ
\left( \boldsymbol{g_{\downarrow 2}} \right)^{( j_{n} - j_{n- 1})}
\right \vert
\boldsymbol{x}
\right)[0],
\end{equation}
where we adopt the convention $j_0 = 0$.
