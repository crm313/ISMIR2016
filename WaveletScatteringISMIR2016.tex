...
which are essential to musical harmony.
...
The chroma representation consists in summing up octaves $u$ at fixed time $t$
and pitch class $q$:
\begin{equation}
\boldsymbol{Y}[t, q]
= 
...
\end{equation}
$\boldsymbol{w}$ is a Gaussian window, whose width $\sigma$
is inversely proportional to the number of bands $K$. The constant $\sigma$
is of the order of one octave for $K=8$, and two octaves for $K=4$.
\begin{equation}
\boldsymbol{Y}[t, q, k]
=
\sum_{u} 
...
\end{equation}
%Introduced by Cho and Bello at the MIREX 
