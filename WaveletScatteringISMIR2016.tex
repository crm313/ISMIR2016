...
\begin{figure}[t]
    ...
    \end{center}
    \protect\caption{
Three voicings of the pitch class set $\mathtt{\{C, E, G, A\}}$.
The first two voicings are identical up to octave transposition
of all the chord factors, and thus have the same chord
label \texttt{A:min7}.
...
Assigning a pure tone to each note would yield the same chroma
representations for all three chords, despite the fact that the
underlying synthetic sounds would be labeled differently by a
human listener.
...
% This article builds upon the system of Cho and Bello \cite{cho2014on},
